...
*   **`.simvision/`, `xcelium.d/`, `xrun.log`:** Arquivos gerados pelas ferramentas de simulação da Cadence (Simvision, Xcelium), evidenciando a execução bem-sucedida das simulações.
*   **`synth.f`, `filelist.f`:** Arquivos de filelist que listam todos os arquivos de origem necessários para a síntese (`synth.f`) e simulação (`filelist.f`).
*   **`can_btu_tb.vcd` e `can_btu_top.sdf`:** `can_btu_tb.vcd` é um dump de simulação no formato Value Change Dump. A presença do arquivo `can_btu_top.sdf` (Standard Delay Format) indica que a simulação pós-síntese (gate-level simulation) foi realizada para verificar o funcionamento do design com os atrasos reais.
